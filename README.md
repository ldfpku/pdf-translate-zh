...
除三平台 CI（引擎 + 示例内容层）外，下列「模型 + AI 工具」组合做过端到端验证。**留出集盲测**：代理只拿到英文 PDF 和一句需求，自己走完定级、翻译、出稿、闸门，再由独立评分脚本复核（方法见 [`tests/agent_eval/`](tests/agent_eval/)）；**开发验证**：用于编写与维护本技能的模型，以「效果展示」两份样例为准，从源文档生成到出稿、闸门、干净目录重跑全程实跑。

| 模型 | AI 工具 | 平台 | 日期 | 技能版本 | 结果 |
|---|---|---|---|---|---|
| Claude Opus 5.5 `claude-opus-5-5` | Claude Cowork（桌面应用，云端会话） | Linux 云端沙箱 · Python 3.11 | 2026-09-24 | 1.1.0 | ✅ 开发验证（非盲测）：[效果展示](#效果展示)两份样例——R 级作业指导书（超分、气泡重绘、图内文字、目录书签）与 P 级三张总装图（注记格、明细表、总注重排、竖排尺寸）——闸门全过、干净目录重跑一致；冒烟测试通过 |
